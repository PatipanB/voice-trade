...
module.exports.signedEndpoint = catchAsync(async (req, res) => {
    const now = (new Date()).valueOf()
    const hash = CryptoJS.HmacSHA256(`timestamp=${now}&recvWindow=60000`, process.env.API_SECRET)
    const signature = CryptoJS.enc.Hex.stringify(hash);
    const params = { timestamp: now, signature, recvWindow: 60000 }
    const { data, status } = await axios.get(req.path === '/account' ? process.env.ACCOUNT : process.env.OPEN_ORDER, { params})
    if (req.path === '/account') data.balances = data.balances.filter(({asset}) => allSymbolList.includes(asset))
    res.json(data)
})
